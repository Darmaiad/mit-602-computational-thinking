...
def r_squared(y, estimated):
    ...
    """
    ys = np.array(y) 
    es = np.array(estimated) 
    numerator = ((ys - es)**2).sum()
    denominator = ((ys - ys.mean())**2).sum()
...
def evaluate_models_on_training(x, y, models):
    ...
    bestEstimation = 0
    for model in models:
        estimation = np.polyval(model, x)
        r2 = r_squared(y, estimation)
        if r2 > bestR2:
            bestR2 = r2
    ...
    pylab.plot(x, y, 'r--', label = 'Data Points')
    pylab.plot(x, bestEstimation, label = 'Best Model')
    pylab.show()

### Begining of program
raw_data = Climate(join(current_dir, "./data.csv"))
