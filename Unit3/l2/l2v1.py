...
# # The two histograms help showcase what the weights argument is used for
# L = [1,1,1,1,2]
# # Shows the number of elements that fall to each bin
# pylab.hist(L)
# # A list of all ones: len(L)*[1] 
# # then divide each element of that array with len(L)
# # factor will show with what we will multiply each element with
# factor = pylab.array(len(L)*[1])/len(L)
# print(factor)
# pylab.figure()
# pylab.hist(L, weights = factor)

# We are using a continuous d5
def plotMeans(numDice, numRolls, numBins, legend, color, style):
    ...
    pylab.hist(
        ...
        color = color, 
        label = legend,
        # An array with the same length as the first arg of hist()
        weights = pylab.array(len(means)*[1])/len(means),
        # Add different style hatchmarks to each bin, in order to better differentiate  
        hatch = style
    )
